...
def make_seed_full(shape, n_channels):
    #seed = np.zeros([shape[0], shape[1], n_channels], np.float32)
    seed = np.ones([shape[0], shape[1], n_channels], np.float32)
    #seed[shape[0]//2, shape[1]//2, 3:] = 1.0
    return seed

def make_custom_seed(shape, n_channels, value=1.0):
    seed = np.zeros([shape[0], shape[1], n_channels], np.float32)
    seed[shape[0]//2, shape[1]//2, 3:] = value
    return seed

def make_custom_seed_full(shape, n_channels, value=1.0):
    #seed = np.zeros([shape[0], shape[1], n_channels], np.float32)
    seed = np.ones([shape[0], shape[1], n_channels], np.float32)
    #seed[shape[0]//4:shape[0]//2:, shape[1]//4:shape[1]//2, 3:] = value
    #seed[shape[0]//2, shape[1]//2, 3:] = value
    seed[shape[0]//2, shape[1]//2, 3:] = value
    seed[shape[0]//4, shape[1]//4, 3:] = value
    seed[shape[0]//2+shape[0]//4, shape[1]//2+shape[1]//4, 3:] = value
    seed[shape[0]//2, shape[1]//2+shape[1]//4, 3:] = value
    seed[shape[0]//2+shape[0]//4, shape[1]//4, 3:] = value
    return seed
